  ...
  #define load_func(type, prefix, xlate_flags) \
    inline type##_t prefix##_##type(reg_t addr, bool require_alignment = false) { \
      ...
      if ((xlate_flags) == 0 && likely(tlb_load_tag[vpn % TLB_ENTRIES] == vpn)) { \
        ...
        ust_record_load(addr, data, size, DATA_SRC_LOAD_TLB_1); \
        return data; \
      } \
      if ((xlate_flags) == 0 && unlikely(tlb_load_tag[vpn % TLB_ENTRIES] == (vpn | TLB_CHECK_TRIGGERS))) { \
        ...
        if (proc) READ_MEM(addr, size); \
        ust_record_load(addr, data, size, DATA_SRC_LOAD_TLB_2); \
        return data; \
      } \
      target_endian<type##_t> res; \
      load_slow_path(addr, sizeof(type##_t), (uint8_t*)&res, (xlate_flags)); \
      if (proc) READ_MEM(addr, size); \
      ust_record_load(addr, from_target(res), size, DATA_SRC_LOAD_SLOW_PATH); \
...
# define WRITE_MEM(addr, val, size) \
  ...
  #define store_func(type, prefix, xlate_flags) \
    void prefix##_##type(reg_t addr, type##_t val) { \
      ...
      reg_t vpn = addr >> PGSHIFT; \
      size_t size = sizeof(type##_t); \
      ust_record_store(addr, val, size, DATA_SRC_STORE); \
  ...
  #define amo_func(type) \
    ...
    type##_t amo_##type(reg_t addr, op f) { \
      try { \
        ...
        auto f_lhs = f(lhs); \
        ust_record_amo(addr, (uint64_t)lhs, (uint64_t)f_lhs, sizeof(lhs)); \
  ...
  inline icache_entry_t* refill_icache(reg_t addr, icache_entry_t* entry)
  {
    ...
    if (tracer.interested_in_range(paddr, paddr + 1, FETCH)) {
      entry->tag = -1;
      tracer.trace(paddr, length, FETCH, NULL);
    }
    return entry;
  }
