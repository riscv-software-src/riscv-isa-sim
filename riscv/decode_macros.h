...
// soft registers
#define MD (p->softRegisters[insn.rd()])
#define MS1 (p->softRegisters[insn.rs1()])
#define MS2 (p->softRegisters[insn.rs2()])
...
#define LOAD_MA_REGISTER(SR, PTR, DTYPE) ({ \
        ...
        for (int i = 0; i < SR.getHeight(); ++i) { \
            for ( int j = 0; j < SR.getWidth(); ++j) { \
                ref[i * SR.width + j] = MMU.load<DTYPE>(addr); \
                addr += step; \
            } \
        } \
    })
...
#define STORE_MA_REGISTER(SR, PTR, DTYPE) ({ \
        ...
        for (int i = 0; i < SR.getHeight(); ++i) { \
            for ( int j = 0; j < SR.getWidth(); ++j) { \
             MMU.store<DTYPE>(addr, ref[i * SR.width + j]); \
            addr += step; \
            } \
        } \
    })
        ...
        for ( int i = 0; i < SRD.getHeight(); ++i ) \
            for ( int j = 0; j < SRD.getWidth(); ++j ) { \
                ...
                for ( int k = 0; k < SR1.getWidth(); ++k ) \
                    c[i * SRD.width + j] += a[i * SR1.width + k] * b[k * SR2.width + j]; \
        } \
    })
        ...
        for ( int j = 0; j < SRD.getWidth(); ++j ) { \
            ...
            for ( int ii = 0; ii < SR2.getHeight(); ++ii ) \
                for ( int jj = 0; jj < SR2.getWidth(); ++jj ) \
                    c[i * SRD.width + j] += a[(i + ii) * SR1.width + (j + jj)] * b[ii * SR2.width + jj]; \
        } \
    })

// end matrix accelerator
