...
tlb_entry_t mmu_t::fetch_slow_path(reg_t vaddr)
{
  ...
  if (unlikely(tlb_insn[vpn % TLB_ENTRIES].tag != (vpn | TLB_CHECK_TRIGGERS))) {
    ...
    if (auto host_addr = sim->addr_to_mem(paddr)) {
      ...
      if (!mmio_fetch(paddr, sizeof(fetch_temp[0]), (uint8_t*)&fetch_temp + (vaddr & (PGSIZE-1))))
        throw trap_instruction_access_fault(proc->state.v, vaddr, 0, 0);
      result = {uintptr_t(&fetch_temp), paddr - (vaddr & (PGSIZE-1))};
    }
  } else {
    result = tlb_insn[vpn % TLB_ENTRIES].data;
  }

  check_triggers(triggers::OPERATION_EXECUTE, vaddr, access_info.effective_virt, from_le(*(const uint16_t*)(result.host_addr + (vaddr & (PGSIZE-1)))));
...
void mmu_t::load_slow_path_intrapage(reg_t len, uint8_t* bytes, mem_access_info_t access_info)
{
  ...
  if (!access_info.flags.is_special_access() && vpn == (tlb_load[vpn % TLB_ENTRIES].tag & ~TLB_CHECK_TRIGGERS)) {
    auto host_addr = (const void*)(tlb_load[vpn % TLB_ENTRIES].data.host_addr + (transformed_addr & (PGSIZE-1)));
...
void mmu_t::store_slow_path_intrapage(reg_t len, const uint8_t* bytes, mem_access_info_t access_info, bool actually_store)
{
  ...
  if (!access_info.flags.is_special_access() && vpn == (tlb_store[vpn % TLB_ENTRIES].tag & ~TLB_CHECK_TRIGGERS)) {
    if (actually_store) {
      auto host_addr = (void*)(tlb_store[vpn % TLB_ENTRIES].data.host_addr + (transformed_addr & (PGSIZE-1)));
...
tlb_entry_t mmu_t::refill_tlb(reg_t vaddr, reg_t paddr, char* host_addr, access_type type)
{
  ...
  tlb_entry_t entry = {uintptr_t(host_addr) - (vaddr & (PGSIZE-1)), paddr - (vaddr & (PGSIZE-1))};
