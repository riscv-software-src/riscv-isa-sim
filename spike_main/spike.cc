...
int main(int argc, char** argv)
{
  ...
  parser.option('h', "help", 0, [&](){help(0);});
  parser.option('d', 0, 0, [&](){debug = true;});
  parser.option('g', 0, 0, [&](){histogram = true;});
  parser.option('l', 0, 0, [&](){log = true;});
#ifdef HAVE_BOOST_ASIO
  parser.option('s', 0, 0, [&](){socket = true;});
#endif
  parser.option('p', 0, 1, [&](const char* s){nprocs = atoul_nonzero_safe(s);});
  parser.option('m', 0, 1, [&](const char* s){cfg.mem_layout = parse_mem_layout(s);});
  // I wanted to use --halted, but for some reason that doesn't work.
  parser.option('H', 0, 0, [&](){halted = true;});
  ...
  parser.option(0, "log-cache-miss", 0, [&](){log_cache = true;});
  ...
  parser.option(0, "dump-dts", 0, [&](){dump_dts = true;});
  parser.option(0, "disable-dtb", 0, [&](){dtb_enabled = false;});
  ...
  parser.option(0, "real-time-clint", 0, [&](){cfg.real_time_clint = true;});
