...
{
  // Allow load of multiple elfs
  // Unfortunately it is not additive now as it clears up the previous loads
  // It is however useful to get symbols
  // The last elf should be the combined elf
  // TODO: make elf load additive
  for(auto const& targ : targs)
  {
    std::map<std::string, uint64_t> csymbols = load_payload(targ, &entry);
    symbols.insert(csymbols.begin(), csymbols.end());
  }
