...
// !!! onemli, manuel olarak commit'leri temizlemek icin
...
static inline reg_t execute_insn_fast_without_clear_commit(processor_t* p, reg_t pc, insn_fetch_t fetch) {
...
void processor_t::step_without_clear_commit(size_t n)
{
  if (!state.debug_mode) {
    ...
    try
    {
      ...
      if (unlikely(slow_path()))
      {
        ...
        while (instret < n)
        {
          ...
          if (debug && !state.serialized)
            disasm(fetch.insn);
          pc = execute_insn_logged_without_clear_commit(this, pc, fetch);
          advance_pc();
        }
      }
      else while (instret < n)
      {
        ...
        for (auto ic_entry = _mmu->access_icache(pc); ; ) {
          ...
          pc = execute_insn_fast_without_clear_commit(this, pc, fetch);
